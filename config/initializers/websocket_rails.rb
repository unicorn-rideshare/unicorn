...
WebsocketRails.setup do |config|
  ...
  redis_url = Settings.redis.url.match(/redis:\/\/(.*):(.*)/i)
  redis_host = redis_url[1].split('@').last
  redis_port = redis_url[2].match(/\d+/)[0].to_i
  redis_password = redis_url[1].split('@').size == 2 ? redis_url[1].split('@')[0].gsub(/:/, '') : nil

  config.redis_options = { host: redis_host,
                           port: redis_port,
