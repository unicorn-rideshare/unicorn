...
describe WorkOrderScheduledJob do
  let(:work_order) { FactoryGirl.create(:work_order, scheduled_start_at: DateTime.now + 5.days) }

  before do
    work_order.config = { customer_communications: { communications_enabled: true, exposes_status_publicly: nil } }
    work_order.save
  end
  ...
  describe '.perform' do
    ...
    it 'should remove any WorkOrderEmailJob instances already enqueued for :scheduled_confirmation emails' do
      ...
      WorkOrderScheduledJob.perform(work_order.id)
    end
    ...
    context 'when the work order config contains a :email_morning_of_reminder_offset' do
      before do
        work_order.config = { customer_communications: { communications_enabled: true, email_morning_of_reminder_offset: 1.day.seconds * -1 } }
        work_order.save
      end

      it 'should enqueue a WorkOrderEmailJob using the work order configuration' do
        expect(Resque).to receive(:enqueue_at).with(anything, WorkOrderEmailJob, work_order.id, :morning_of_reminder)
        WorkOrderScheduledJob.perform(work_order.id)
      end
    end
    ...
    context 'when the work order, customer, and company config does not contain a :email_morning_of_reminder_offset' do
      before do
        [work_order, work_order.customer, work_order.company].each do |obj|
          obj.config = { customer_communications: { communications_enabled: true, email_morning_of_reminder_offset: nil } }
          obj.save
        end
      end

      it 'should not enqueue a WorkOrderEmailJob using the work order configuration' do
        expect(Resque).not_to receive(:enqueue_at).with(anything, WorkOrderEmailJob, work_order.id, :morning_of_reminder)
        WorkOrderScheduledJob.perform(work_order.id)
      end
    end
    ...
    context 'when the work order config contains a :email_reminder_offset' do
      before do
        work_order.config = { customer_communications: { communications_enabled: true, email_reminder_offset: 1.day.seconds * -1 } }
        work_order.save
      end

      it 'should enqueue a WorkOrderEmailJob using the work order configuration' do
        expect(Resque).to receive(:enqueue_at).with(anything, WorkOrderEmailJob, work_order.id, :reminder)
        WorkOrderScheduledJob.perform(work_order.id)
      end
    end
    ...
    context 'when the work order, customer, and company config does not contain a :email_reminder_offset' do
      before do
        [work_order, work_order.customer, work_order.company].each do |obj|
          obj.config = { customer_communications: { communications_enabled: true, email_reminder_offset: nil } }
          obj.save
        end
      end
    ...
    context 'when the work order config contains a :email_scheduled_confirmation_offset' do
      before do
        work_order.config = { customer_communications: { communications_enabled: true, email_scheduled_confirmation_offset: 2.days.seconds * -1 } }
        work_order.save
      end
      
      it 'should enqueue a WorkOrderEmailJob per the work order configuration' do
        expect(Resque).to receive(:enqueue_at).with(anything, WorkOrderEmailJob, work_order.id, :scheduled_confirmation)
        WorkOrderScheduledJob.perform(work_order.id)
      end
    end
    ...
    context 'when the work order config does not contain a :email_scheduled_confirmation_offset' do
      before do
        [work_order, work_order.customer, work_order.company].each do |obj|
          obj.config = { customer_communications: { communications_enabled: true, email_scheduled_confirmation_offset: nil } }
          obj.save
        end
      end
