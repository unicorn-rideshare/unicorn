...
EOF
  ...
  def broadcast_tx
    ...
    status, _, resp = BlockchainService.execute_contract(jwt, wo_contract_id, { wallet_id: wallet_id, value: 0, method: 'createWorkOrder', params: [self.id] })
    tx = nil
    if status == 202
      if resp['transaction']
        tx = resp['transaction']
        ...
        while tx.nil?
          sleep(1.0)
          status, _, resp = BlockchainService.transaction_details(jwt, tx_ref)
